 ...
 */
static void onTCPConnect(Socket_t socket, BaseType_t isConnected)
{
	if (!isConnected)
	{
		// A TCP connection was closed.  Close the corresponding
		// firewall hole.
		// TODO What happens if this races with a call to
		// `FreeRTOS_closesocket`?
...
F_TCP_UDP_Handler_t onTCPConnectCallback = {onTCPConnect};
                               ...
                               uint16_t       *port)
{
	...
	with_sealed_socket(
	  [&](SealedSocket *listeningSocket) {
		  ...
		  {
			  *port = FreeRTOS_ntohs(addressTmp.sin_port);
		  }
